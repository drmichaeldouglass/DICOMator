...
- **Voxelization control**
  ...
  - A mesh made of several closed surfaces is filled surface by surface: a surface nested inside another makes a cavity, and surfaces that merely overlap (organs joined into one object) are merged rather than cancelling each other
...
- **Synthetic artifact suite** (image series only)
  ...
  - MRI applies, in order: motion blur, geometric distortion (gradient non-linearity and B0 off-resonance, with signal-conserving intensity pile-up), Gibbs/truncation ringing, coil-shaped bias-field shading, and Rician noise, after which the magnitude volume is clamped to non-negative values
  ...
  - A stored artifact seed makes repeated exports reproducible. Each artifact draws from its own random stream, so switching one artifact on or off never changes another's pattern; every 4D phase gets a deterministic sub-seed, except detector rings, which are a scanner property and stay identical across phases
...
4. Click **Export DICOM**.
   ...
   - When DRR is enabled, the voxelized HU volume is projected from the active camera into a single DICOM secondary-capture image per phase. The DRR always uses CT numbers: for an MR export each mesh's material preset supplies its CT value (custom-intensity meshes are treated as water, 0 HU, with a warning) and the background is air, so the projection stays physically meaningful.
...
- **RT Structure Set output**
  ...
  - Only closed planar loops with three or more points are written; open or degenerate edge chains are discarded and reported as warnings
  - ROIs with a loop lying entirely inside another loop on any slice (a cavity) use `CLOSEDPLANAR_XOR` for every contour in that ROI, preserving holes regardless of loop winding. All other ROIs, including ones with several separate loops, use the more widely supported `CLOSED_PLANAR`.
